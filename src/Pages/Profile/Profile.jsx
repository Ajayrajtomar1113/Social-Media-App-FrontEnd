        ...
        <div className="px-3 sm:px-5 flex flex-col sm:flex-row sm:justify-between">
          ...
          <div className="mt-2 sm:mt-4">
            {isOwnProfile ? (
              ...
              <Button onClick={() => deleteProfile(currentUser?.id)} size="small" variant='outlined'>
                Delete
              </Button>
            ) : (
              <Button
                size="small"
                variant='outlined'
                onClick={() => handleFollow(auth?.profile?.id)}
              >
                {auth?.user?.followings?.includes(auth?.profile?.id)
                  ? "Unfollow"
                  : "Follow"}
              </Button>
            )}
